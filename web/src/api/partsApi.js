...
export async function createPart({ name, category, stockQuantity, isAvailable, imageUrl }) {
  const response = await fetch(`${API_BASE_URL}/api/parts`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name, category, stockQuantity, isAvailable, imageUrl }),
  })
  return handleResponse(response)
}

export async function updatePart(id, { name, category, stockQuantity, isAvailable, imageUrl }) {
